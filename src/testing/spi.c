...
int DAC_set(uint16_t data); //ahe LP 
...
// ahe - pls test with LP
	printf("%d\n",data);
	printf("H:%x\nL:%x\n",(uint8_t)data&0xff00,(uint8_t)data&0x00ff);
	char data_buffer[3] = {0x00,data&0xff00,data&0x00ff}; // buffer to hold the data

    int write_spi = write(spi_fd, data_buffer,3);
